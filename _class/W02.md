...
Hands-on with the development workflow. Terminal basics, Git, and Node.js setup. Editing shader exercise files and seeing immediate results. First vibecoding session: using an LLM to fix aspect-ratio distortion (and learning to ask for minimal, testable changes). We reinforce the course's AI approach: AI helps you write and revise code, but you remain the author responsible for choices, attribution, and understanding.
...
1. **Aspect-Ratio Fix Study:** Use an LLM to correct aspect-ratio distortion and document the interaction using the Prompt Log template. Commit your final working solution.
2. **Share Your First Shader Video:** Export a short clip and post it with a brief technical note (what parameter you changed and why).
