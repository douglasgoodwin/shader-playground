...
void main() {
    ...
    for (float i = 0.0; i < 150.0; i += 1.0) {
        ...
        vec2 seed = hash21(id * 13.7);
        float phase = hash11(id * 9.1) * TAU;
        float blinkRate = mix(0.8, 2.2, hash11(id * 5.3)); // Hz-ish
        float size = mix(0.004, 0.010, hash11(id * 2.7));  // core radius in uv units
        float halo = size * 6.0;
        ...
        float f1 = mix(0.2, 0.7, hash11(id * 1.1));
        float f2 = mix(0.15, 0.55, hash11(id * 1.9));
        ...
        float eventRate = mix(0.15, 0.45, hash11(id * 7.7));
        float eventT = t * eventRate + hash11(id * 11.3) * 10.0;
        float k = fract(eventT);
        float burst = exp(-k * 18.0);

        // Random burst direction
        vec2 dir = normalize(hash21(id * 23.9) - 0.5);
