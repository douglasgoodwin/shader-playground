...
void main() {
    ...
    vec3 color = vec3(0.01, 0.01, 0.008);
    ...
    if (d < MAX_DIST) {
        ...
        // Mottled marble
        vec3 warmWhite = vec3(0.85, 0.8, 0.72);
        vec3 cream = vec3(0.78, 0.72, 0.6);
        vec3 coolGrey = vec3(0.6, 0.58, 0.56);
        vec3 pinkish = vec3(0.82, 0.7, 0.65);
        vec3 matColor;
        float specStr = 0.2;
        float pick = h1;
        if (pick < 0.4) {
            matColor = mix(warmWhite, cream, h2);
        } else if (pick < 0.65) {
            matColor = mix(cream, pinkish, h2);
        } else if (pick < 0.85) {
            matColor = mix(warmWhite, coolGrey, h2 * 0.6);
        } else {
            matColor = mix(coolGrey, cream, h2);
        }
        matColor *= 0.88 + h3 * 0.24;

        vec3 ambient = 0.12 * matColor;
        vec3 diffuseLight = diff * lightColor * matColor;
        vec3 specLight = spec * specStr * lightColor;

        color = (ambient + diffuseLight + specLight) * ao;

        // Fill light from right
        vec3 fillDir = normalize(vec3(4.0, 2.0, -2.0) - p);
        float fillDiff = max(dot(n, fillDir), 0.0);
        color += fillDiff * 0.06 * vec3(0.5, 0.55, 0.65) * matColor;

        // Cool rim
        float rim = pow(1.0 - max(dot(smoothN, viewDir), 0.0), 3.0);
        color += rim * 0.04 * vec3(0.35, 0.45, 0.55);
    }

    color *= vec3(1.05, 0.95, 0.85);

    vec2 vUv = gl_FragCoord.xy / u_resolution;
    float vignette = 1.0 - 0.6 * pow(length(vUv - 0.5) * 1.4, 2.0);
    color *= vignette;
